...
export default function UserTable({ currentItems }: adUserTableProp) {
  ...
  const [current, setCurrentItems] = useState<IMockUser[]>([]);
              ...
              <Tr key={index} bg={"white"} mb={10} border={"none"}>
                <Td>
                  <div className="w-full flex items-center gap-2">
                    ...
                    <p className="font-semibold text-[#FF9C50]">{user?.name}</p>
                  </div>
                </Td>
                <Td>
                ...
                <Td>
                  <p className="font-[500]">{user?.sport}</p>
                </Td>
                <Td>
