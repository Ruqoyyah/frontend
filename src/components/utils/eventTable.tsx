...
export default function EventTable({ currentItems }: adUserTableProp) {
  const router = useRouter();
  const [pageCount, setPageCount] = useState(0);
  const [itemOffset, setItemOffset] = useState<number>(0);
  const [limit, setLimit] = useState<number>(10);
  const [current, setCurrentItems] = useState<IEvent[]>([]);
  ...
  const deleteEvent = async (id: number) => {
    setLoading(true);
    try {
      const res = AdminServices.deleteSingleEvent(id);
      if (res != undefined) {
        setLoading(false);
        toast({
          status: "success",
          ...
          <Tbody>
            {current?.map((user, index: number) => (
              <Tr key={index} bg={"white"} mb={10} border={"none"}>
                ...
                <Td>
                  <p className="font-[500]">{user.sport.sportName}</p>
                </Td>
                <Td>
